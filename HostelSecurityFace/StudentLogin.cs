...
namespace HostelSecurityFace
{
    public partial class StudentLogin : Form
    {
        ...
        {
            con.Open();
            cmd = new SqlCommand("select * from regtb where regno='" + textBox1.Text + "' and Password='" + textBox2.Text + "'", con);
            SqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {

                StudentHome sh = new StudentHome();
                sh.sname = dr["name"].ToString();
                sh.rname =textBox1.Text;
                sh.mob = dr["Mobile"].ToString();
